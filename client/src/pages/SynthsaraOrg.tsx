...
const categoryLabels: Record<string, { label: string; tone: string }> = {
  ...
  meta: { label: "Meta", tone: "bg-slate-500/15 text-slate-100" },
  protocol: { label: "Protocol", tone: "bg-purple-500/15 text-purple-100" },
  dashboard: { label: "Dashboard", tone: "bg-sky-500/15 text-sky-100" },
  governance: { label: "Governance", tone: "bg-emerald-500/15 text-emerald-100" },
  marketplace: { label: "Marketplace", tone: "bg-amber-500/15 text-amber-100" },
  token: { label: "Token", tone: "bg-cyan-500/15 text-cyan-100" },
  tool: { label: "Tool", tone: "bg-blue-500/15 text-blue-100" },
  ai: { label: "AI", tone: "bg-fuchsia-500/15 text-fuchsia-100" },
  contract: { label: "Contract", tone: "bg-rose-500/15 text-rose-100" },
  page: { label: "Page", tone: "bg-slate-500/15 text-slate-100" },
};
      ...
      <div className="flex items-start gap-3 p-4">
        <div
          className={`p-2 rounded-lg border border-white/10 ${
            isImage ? "bg-sky-500/10" : isCode ? "bg-slate-500/15" : "bg-amber-500/10"
          }`}
        >
          {isImage ? (
            <ImageIcon className="w-4 h-4 text-sky-200" />
          ) : isCode ? (
            <FileCode2 className="w-4 h-4 text-slate-100" />
          ) : (
...
export default function SynthsaraOrg() {
  const { data: docs = [], isLoading } = trpc.synthsaraOrg.list.useQuery();
  ...
  const selectedDocQuery = trpc.synthsaraOrg.get.useQuery(
    { id: selectedId ?? "" },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#06070f] via-[#0b0f1c] to-[#05040c] text-white relative overflow-hidden">
      <div className="pointer-events-none absolute inset-0 opacity-60">
        <div className="absolute -left-24 top-10 h-80 w-80 rounded-full blur-3xl bg-purple-500/10" />
        <div className="absolute right-0 top-1/2 h-96 w-96 -translate-y-1/2 rounded-full blur-3xl bg-emerald-500/10" />
        <div className="absolute -right-16 bottom-10 h-64 w-64 rounded-full blur-3xl bg-amber-500/10" />
      </div>
        ...
        <header className="container py-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
          <div className="space-y-3">
            <div className="flex items-center gap-3 text-xs uppercase tracking-[0.25em] text-white/60">
              ...
              Synthsara.org Source Deck
            </div>
            <h1 className="text-4xl md:text-5xl font-serif font-semibold">
              Governance + Protocol Library
            </h1>
            <p className="text-white/70 max-w-2xl leading-relaxed">
              Front-end flows, governance contracts, and protocol texts from Synthsara.org.
              Stream the raw TSX/Solidity/markdown, inspect the Universal Diamond Standard,
              and reuse these modules inside the Codex runtime.
            </p>
          </div>
            ...
            <Card className="bg-white/5 border-white/10">
              ...
              <CardContent className="flex items-center gap-2">
                <div className="text-4xl font-bold">
                  {docCountByCategory.contract ?? 0}
                </div>
                <Badge variant="outline" className="border-rose-400/40 text-rose-100">
                  solidity
                </Badge>
              </CardContent>
            </Card>
            ...
            <CardHeader className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                <CardTitle className="text-lg font-semibold">
                  Synthsara.org Library
                </CardTitle>
                <p className="text-sm text-white/60">
                  Filter UI modules, contracts, and protocol texts. Click an entry to render the source.
                </p>
              </div>
                ...
                <div className="rounded-[14px] border border-white/5 bg-black/30 backdrop-blur-sm p-6 h-[560px] flex flex-col gap-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {selectedDoc?.format === "image" ? (
                      ...
                      <FileCode2 className="w-5 h-5 text-slate-100" />
                    ) : (
                  ...
                  <div className="flex-1 overflow-hidden">
                    {selectedDocQuery.isLoading ? (
                      ...
                      <div className="h-full flex flex-col items-center justify-center gap-3 text-white/60">
                        ...
                        Choose a Synthsara.org entry to render its source.
                      </div>
                    ) : selectedDoc.format === "image" ? (
